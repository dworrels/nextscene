...
export const metadata: Metadata = { title: "Review Watchlist — NextScene" };

export default async function WatchlistReviewPage() {
  ...
  return <main className="pb-24">
    ...
    <section className="page-width pt-[84px] max-[760px]:pt-11 pb-6">
      <BackButton className="mb-4 inline-flex h-9 w-9 items-center justify-center rounded-full border border-line bg-soft text-ink hover:bg-line" fallbackHref="/watchlist" ariaLabel="Back to your watchlist" />
      <h1 className="m-0 text-[clamp(36px,5vw,60px)] font-bold leading-[0.98] tracking-[-0.02em]">Review &amp; Manage</h1>
      <p className="mt-4 text-sm text-muted">
        {rows.length === 0
          ? "You haven't imported a watchlist yet."
          : unmatched.length > 0
    ...
    {rows.length > 0 ? <section className="page-width flex flex-wrap items-center gap-3 pb-10">
      <Link className="inline-flex min-h-11 items-center rounded-full border border-line px-5 py-2 text-xs font-semibold text-ink hover:bg-soft" href="/watchlist">View watchlist</Link>
    ...
    {unmatched.length > 0 ? <section className="page-width">
      ...
      <div className="flex flex-col gap-2">
        {unmatched.map((row) => <WatchlistRepairRow imdbId={row.imdbId} mediaType={row.mediaType} title={row.title} key={row.imdbId} />)}
      </div>
    </section> : null}
  </main>;
}
