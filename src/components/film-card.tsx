...
export function FilmCard({ movie, badge, predictedBadge, fluid = false }: { movie: MediaItem; badge?: string; predictedBadge?: string; fluid?: boolean }) {
  ...
  const widthClass = fluid ? "w-full" : "w-[148px] flex-none";
  const posterClass = fluid ? "aspect-[2/3] w-full" : "h-[222px] w-[148px]";
  ...
  return <Link className={`group relative block ${widthClass}`} href={href} aria-label={`${movie.title} (${movie.year})`} prefetch={false}>
    <div className={`relative ${posterClass} overflow-hidden rounded-xl bg-soft transition-transform duration-300 ease-out group-hover:scale-[1.04] group-hover:shadow-[0_16px_32px_rgba(0,0,0,.4)]`}>
      {movie.posterUrl ? <Image alt="" className="object-cover" fill loading="lazy" sizes={fluid ? "(max-width: 479px) 46vw, (max-width: 759px) 30vw, 148px" : "148px"} src={movie.posterUrl} /> : <div className="grid h-full w-full place-items-center bg-[linear-gradient(135deg,var(--color-soft),var(--color-line))] p-4 text-center text-sm font-semibold leading-snug text-muted">{movie.title}</div>}
    </div>
    {badge ? <span className="absolute left-2 top-2 rounded-full bg-black/70 px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.04em] text-white backdrop-blur-sm">{badge}</span> : null}
